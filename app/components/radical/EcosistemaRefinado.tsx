...
const ecosystemServices = [
  ...
  },
  {
    ...
    color: 'from-blue-400/20 to-blue-700/10',
  ...
  },
  {
    ...
    color: 'from-blue-700/20 to-blue-400/10',
...
const ServiceCard = ({ service, index }: { service: typeof ecosystemServices[0], index: number }) => (
  ...
  >
    ...
    <div className="relative z-10">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-white">{service.metrics.value}</div>
          <div className="text-xs text-gray-400 uppercase tracking-wide">{service.metrics.label}</div>
        </div>
      </div>

      {/* Título y subtítulo */}
      <div className="mb-6">
        <div className="text-xs text-blue-400 uppercase tracking-widest font-medium mb-2">
          {service.subtitle}
        </div>
        <Heading level={3} size="heading" className="mb-4 group-hover:text-blue-100 transition-colors duration-300">
          {service.title}
        </Heading>
        <Text className="leading-relaxed group-hover:text-gray-200 transition-colors duration-300">
          {service.description}
        </Text>
      </div>
      ...
      <div className="mb-8">
        <ul className="space-y-2">
          {service.features.map((feature, idx) => (
            <li key={idx} className="flex items-center gap-3 text-sm text-gray-400 group-hover:text-gray-300 transition-colors duration-300">
      ...
      <Button 
        ...
        className="w-full group-hover:border-blue-400 group-hover:text-blue-400"
      >
        Conocer Más
      </Button>
    </div>
    ...
    {/* Decoración esquina */}
    <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-blue-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
  </Card>
)
...
const EcosistemaRefinado = () => {
  return (
    <Section background="primary" padding="xl" id="ecosistema">
      {/* Header de sección */}
      <SectionHeader
      ...
      {/* Estadísticas del ecosistema */}
      <div className="bg-gradient-to-r from-blue-500/10 via-blue-600/5 to-blue-500/10 border border-blue-500/20 p-8 lg:p-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {ecosystemStats.map((stat, index) => (
            <div
              key={stat.label}
              className="bg-black/30 backdrop-blur-sm border border-blue-500/20 p-6 text-center group hover:border-blue-500/40 hover:bg-black/50 transition-all duration-300 fade-in"
            ...
            >
              ...
              <div className="text-sm text-gray-400 uppercase tracking-wide font-medium leading-tight">
                {stat.label}
              </div>
              
              {/* Línea decorativa */}
              <div className="w-full h-px bg-gradient-to-r from-transparent via-blue-500/40 to-transparent mt-4 group-hover:via-blue-500/60 transition-all duration-300"></div>
            </div>
          ))}
        </div>
      </div>
